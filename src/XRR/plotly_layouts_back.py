...
class plotly_layouts:
	...
	def refl2(self, dtick_x = 0.1, x0 = 0, x1=10, y0 = -20, y1= 20, numb_xaxis_minorticks=0, numb_yaxis_minorticks = 10, every_nth_logtick=1):

			ytickvals, yticktext = make_logtick_array(x0=-30, x1=30, every_nth_val = every_nth_logtick, ticksbetween = numb_yaxis_minorticks)
			xtickvals, xticktext = self.make_tick_array(x0 = x0, x1 = x1, dtickMain = dtick_x, ticksbetween=numb_xaxis_minorticks)
			if self.unit_mode == 'slash':
				ttext_x = '<i>q</i><sub>z</sub>/&#8491;<sup>-1</sup>'
				ttext_y = '<i>I</i>/arb. u.'
				ttext_minor_x = '<i>q</i><sup>2</sup><sub>z</sub>/&#8491;<sup>-2</sup>'
			elif self.unit_mode == 'bracket':
				ttext_x = '<i>q</i><sub>z</sub> <span>[<span>&#8491;</span><sup>-1</sup></span>]'
				ttext_y = r'$I\,\left[\text{arb. u.}\right]$'
				ttext_minor_x = '<i>q</i><sub>z</sub><sup>2</sup>&#8201;[&#8491;<sup>-2</sup>]'

			layout_refl = go.Layout(
		    title = dict(text = None, font = dict(size = self.titleFS, color = self.FontColors, family = self.FontFamily),
		    	xanchor = self.title_xanchor, x = self.title_x, y = self.title_y), 
		    yaxis = dict(
		        title = dict(text = ttext_y, standoff= self.title_standoff_y,
		        	font = dict(size=self.titleFS_y, color = self.FontColors, family = self.FontFamily)
		        	),
		        type='log', showexponent='all', exponentformat='power', tickmode = 'array',
		        visible=True, linecolor='black', automargin=False, zeroline=True, showline=True, showgrid=self.showgrid, gridwidth=1,
		        gridcolor='lightgrey', ticks='inside', showticklabels=True, tickvals = ytickvals,
		        ticktext = yticktext,tickfont = dict(size=self.tickFS, color = self.FontColors), ticklen = self.ticklen, tickwidth = self.tickwidth -2,
		        mirror = 'all'
		        ),
		    xaxis = dict(
		        title = dict(text = ttext_x, standoff= self.title_standoff_x,
		        	font = dict(size = self.titleFS_x, color = 'black', family = self.FontFamily)
		        	),
		        type='linear', visible=True, linecolor='black', showgrid=self.showgrid,automargin=True, gridwidth=1, gridcolor='lightgrey', tickmode='array',
		        zeroline=True, showline=True, ticks='inside', tickvals=xtickvals, ticktext=xticktext,showticklabels=True, rangemode='nonnegative',
		        tickfont = dict(size=self.tickFS, color = self.FontColors), ticklen = self.ticklen, tickwidth = self.tickwidth,
		        mirror = 'all',
		    ), 
		    xaxis2 = dict(
		        title = dict(text = ttext_minor_x, standoff= 0,
		        	font = dict(size = 20, color = 'black', family = self.FontFamily)
		        	),
		        domain = [0.5, 0.97], anchor = 'y2',
		        type='linear', visible=True, linecolor='black',automargin=True, gridwidth=1, gridcolor='lightgrey', tickmode='array',
		        zeroline=True, showline=True, ticks='inside', tickvals=xtickvals, ticktext=xticktext,showticklabels=True, rangemode='nonnegative',
		        tickfont = dict(size=16, color = self.FontColors), ticklen = self.ticklen, tickwidth = self.tickwidth,
		        mirror = 'ticks',
		        ),
		    yaxis2 = dict(
		        title = dict(text = ttext_y, standoff= 0,
		        	font = dict(size=20, color = self.FontColors, family = self.FontFamily)
		        	),
		        domain = [0.45, 0.97], anchor = 'x2', side='left',
		        type='log', showexponent='all', exponentformat='power', tickmode = 'array',
		        visible=True, linecolor='black', automargin=False, ticks='inside', showticklabels=True, tickvals = ytickvals,
		        ticktext = yticktext,tickfont = dict(size=16, color = self.FontColors), ticklen = self.ticklen, tickwidth = self.tickwidth -2,
		        mirror = 'all'
		        ),
		    legend = dict(
		    	title=dict(
		    ...
		    font = dict(size = self.traceFS_legend, color = self.FontColors, family = self.FontFamily),
		    	yanchor='top', xanchor='right', y=0.99, x=0.98, borderwidth=0, bgcolor = self.legendbgcolor,
		    ),
	...
	def sbs_subplots(self):
	    ...
	    yaxis = dict(
	        ...
	        type='log', exponentformat='power', showexponent='first', visible=True, linecolor='black', zeroline=True, showline=True, showgrid=self.showgrid, gridwidth=1,
		        gridcolor='lightgrey', ticks = 'inside', nticks = 5,tickfont = dict(size=self.tickFS, color = self.FontColors), ticklen = self.ticklen, tickwidth = self.tickwidth,
		        minor =  dict(ticks='inside', ticklen=self.ticklen -2, tickwidth = self.tickwidth -2, tickmode = 'auto', nticks = 5),
		        mirror = 'all',
	        ),
	    xaxis = dict(
	        title = dict(text = ttext_x, standoff= 0,
	        	font = dict(size = self.titleFS_x, color = 'black', family = self.FontFamily)
	        	),
	        type='linear', visible=True, linecolor='black', showgrid=self.showgrid, automargin=True, gridwidth=1, gridcolor='lightgrey', tickmode='auto',
	        zeroline=True, showline=True, ticks='inside', showticklabels=True,
	        tickfont = dict(size=self.tickFS, color = self.FontColors), ticklen = self.ticklen, tickwidth = self.tickwidth,
	        mirror = 'all', domain = [0, 0.48],
	    ), 
	    yaxis2 = dict(anchor = 'x2', side = 'right',
	        title = dict(text = ttext_y_html_eldens,
	        	font = dict(size=self.titleFS_y, color = self.FontColors, family = self.FontFamily),
	        	standoff = self.title_standoff_y-self.title_standoff_y),
	        	type='linear' , visible=True, linecolor='black',zeroline=True, showline=True, automargin = True, tickmode ='auto',
	        	showgrid=self.showgrid, gridwidth=1, gridcolor='lightgrey', ticks='inside', showticklabels=True, mirror = 'all',
	        	tickfont = dict(size=self.tickFS, color = self.FontColors),
	    ),
	    xaxis2 = dict(
	        title = dict(text = ttext_x_html_eldens, standoff= self.title_standoff_x-self.title_standoff_x,
	        	font = dict(size = self.titleFS_x, color = 'black', family = self.FontFamily),
	        	),
	        tickfont = dict(size=self.tickFS, color = self.FontColors),
	        visible=True, linecolor='black', showgrid=self.showgrid, automargin = False, mirror = 'all',
	        gridwidth=1, gridcolor='lightgrey', zeroline=True, showline=True, ticks='inside',
	        domain = [0.52, 1],),
	    legend = dict(
	    	title=dict(
	    		text=None, font = dict(
